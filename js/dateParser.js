...
export function resolveTimeFromText(input, fallback = null) {
  ...
  for (const word of wordKeys) {
    const regex = new RegExp(`\\b(?:as|a|pelas?)?\\s*${word}\\s*(?:horas?)?(?:\\s+(?:da|de|pela)\\s+(?:manha|tarde|noite))?\\b`);
    if (regex.test(text)) {
...
function normalizeTime(hour, minute, text) {
  const rawHour = hour;
  const hasMorning = /\b(?:da|de|pela) manha\b/.test(text);
  const hasAfternoon = /\b(?:da|de|pela) tarde\b/.test(text);
  const hasNight = /\b(?:da|de|pela) noite\b/.test(text);
...
export function resolveAmbiguousPeriod(time, periodText = "") {
  ...
  const wantsMorning = /\b(manha|de manha|da manha|cedo|am)\b/.test(period);
  const wantsNight = /\b(noite|da noite|de noite|tarde|da tarde|de tarde|pm)\b/.test(period);
