...
function normalizeLexeme(value = "") {
  return normalizeText(value).replace(/[^a-z0-9\s\-\u3040-\u30ff\u3400-\u9fff]/g, " ").replace(/\s+/g, " ").trim();
}
...
// === V0.4 COMPLETE EXPORTS ===

for (let number = 0; number <= 999; number++) {
  addLexeme(String(number), "number", "valor numerico", "multi", number);
}

function tokenizeForLexicon(input = "") {
  const normalized = normalizeLexeme(input);
  ...
  for (const key of keys) {
    ...
    if (regex.test(normalized) && !consumed.has(key)) {
      consumed.add(key);
      found.push({ key, ...LEXICON.get(key) });
    }
  }
  return found;
}

export function analyzeSemanticIntent(input = "") {
  const matches = tokenizeForLexicon(input);
  const tags = new Map();
  for (const match of matches) {
    for (const tag of match.tags) tags.set(tag, (tags.get(tag) ?? 0) + 1);
  }
  const has = (tag) => (tags.get(tag) ?? 0) > 0;
  let intent = "unknown";
  if (has("greet_person")) intent = "greet_person";
  else if (has("ask") && has("user")) intent = "ask_user";
  else if (has("user") && has("have")) intent = "user_has";
  else if (has("assistant") && has("have")) intent = "assistant_has";
  else if (has("play") && has("music")) intent = "play_music";
  else if (has("near") && has("gas_station")) intent = "near_gas_station";
  else if (has("near") && has("pharmacy")) intent = "near_pharmacy";
  else if (has("near") && has("hospital")) intent = "near_hospital";
  else if (has("near") && has("supermarket")) intent = "near_supermarket";
  else if (has("near") && has("restaurant")) intent = "near_restaurant";
  else if (has("internet") && (has("information") || has("what") || has("who") || has("where"))) intent = "internet_search";
  else if (has("story") && has("answer")) intent = "tell_story";
  else if (has("conversation")) intent = "conversation";
  else if (has("greeting")) intent = "greeting";
  else if (has("thanks")) intent = "thanks";
  else if (has("anime")) intent = "anime";
  else if (has("information") || has("who") || has("what") || has("why") || has("how") || has("where") || has("when")) intent = "information";
  return {
    intent,
    confidence: matches.length ? Math.min(1, 0.22 + matches.length * 0.08) : 0,
    matches,
    tags: Object.fromEntries(tags),
    dominantTags: [...tags.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8)
  };
}

export function getLexiconEntry(term = "") {
  return LEXICON.get(normalizeLexeme(term)) ?? null;
}
...
export function getLexiconStats() {
  ...
  for (const entry of LEXICON.values()) {
    for (const language of entry.languages) languageCounts[language] = (languageCounts[language] ?? 0) + 1;
    for (const tag of entry.tags) tagCounts[tag] = (tagCounts[tag] ?? 0) + 1;
  }
  return { entries: LEXICON.size, languages: languageCounts, tags: tagCounts };
}

export function semanticWords(input = "") {
  return tokenizeForLexicon(input);
}
