...
import { answerAnimeQuestion, answerSystemQuestion, isLikelyAnimeTopic } from "./knowledgeBase.js";
...
export class JordanAssistant {
  constructor(calendar, memory, stories) {
  ...
  async execute(rawInput) {
    ...
    if (!text) return this.response("Tô ouvindo!", { mood: "excited", casual: true });
    ...
    if (isLikelyAnimeTopic(text)) {
      return this.response(
        "Eu reconheci que é sobre anime, mas esse detalhe ainda não está no meu conhecimento offline. Quando você me der acesso à internet, esse tipo de pergunta vai ficar muito mais forte. Por enquanto, tenta citar o personagem e o que exatamente quer saber.",
        { topic: "anime" }
      );
    }
    ...
    if (/\?$/.test(original.trim())) {
      return this.response(
        "Essa eu ainda não sei responder com segurança só com meu conhecimento local. Mas pode continuar falando comigo; se for algo que você me ensinou, agenda, sistema ou anime que eu conheço, eu tento responder.",
        { understood: false, casual: true }
      );
    }

    return this.response(
      "Saquei. Quer continuar me contando ou quer que eu faça alguma coisa com isso?",
      { understood: false, casual: true }
    );
  }

  response(text, extra = {}) {
    let styled = this.applySpeechStyle(text);
  ...
  isGreeting(text) {
    return /^(oi|ola|bom dia|boa tarde|boa noite|e ai|opa|salve)\b/.test(text);
  }
  ...
  isOpenCalendarIntent(text) {
    return /\b(abra|abre|mostrar|mostra|ver|quero ver|acessar|acesse)\b.*\b(calendario|agenda)\b/.test(text);
  }

  isOpenMemoryIntent(text) {
    return /\b(abra|abre|mostrar|mostra|ver|quero ver|acessar|acesse)\b.*\b(memoria|o que voce sabe)\b/.test(text);
  }

  isCapabilitiesIntent(text) {
    return /\b(o que voce pode fazer|oque voce pode fazer|quais comandos|mostrar comandos|mostre seus comandos|tutorial|suas funcoes|suas funcionalidades)\b/.test(text);
  }

  isEmergencyIntent(text) {
    return /^(ajuda|me ajuda|socorro|emergencia|preciso de ajuda|preciso de socorro)$/.test(text.trim());
  }
  ...
  greeting() {
    ...
    if (this.personality === "introverted") return this.response(`${greeting}. Tô aqui.`);
    return this.response(`${greeting}! Tô aqui e pronta. O que a gente vai fazer?`, { mood: "excited", casual: true });
  }
  ...
  async handlePendingAction(original, text) {
    ...
    if (pending.type === "capture-story") {
      ...
      const sanitized = await this.stories.saveUserStory(original.replace(/^\s*jordan[\s,:-]*/i, ""));
      this.context.pendingAction = null;
      return this.response(
